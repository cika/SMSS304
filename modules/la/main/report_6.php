...
echo "<tr align='center'><td><font color='#006666' size='3'><strong>ทะเบียนการลากิจ ลาป่วย ลาคลอด</strong></font></td></tr>";
...
echo "<Tr ><Td colspan='2' align='right'><INPUT TYPE='button' name='smb' value='<<กลับหน้าก่อน' onclick='location.href=\"?option=la&task=main/report_6&page=$_GET[page]&person_id=$user\"'></Td></Tr>";
...
//ส่วนของการแยกหน้า
$sql="select id from la_main where person_id='$user' and la_type BETWEEN '1' AND '3'";
$dbquery = mysqli_query($connect,$sql);
$num_rows = mysqli_num_rows($dbquery);

$pagelen=15;  // 1_กำหนดแถวต่อหน้า
$url_link="option=la&task=main/report_6&person_id=$_GET[person_id]";  // 2_กำหนดลิงค์ฺ
...
echo "<Tr ><Td align='right'><INPUT TYPE='button' name='smb' value='<<กลับหน้าสถิติการลา' onclick='location.href=\"?option=la&task=main/report_4\"'></Td></Tr>";
echo "</table>";
$sql="select * from la_main where person_id='$user' and la_type BETWEEN '1' AND '3' order by id limit $start,$pagelen";
$dbquery = mysqli_query($connect,$sql);
...
echo "</Td>";
echo "<Td valign='top' align='center'><a href=?option=la&task=main/report_6&index=7&id=$id&person_id=$user&page=$page><img src=images/browse.png border='0' alt='รายละเอียด'></Td>";
echo "</Tr>";
...
function goto_url(val){
	if(val==0){
		callfrm("?option=la&task=main/report_6");   // page ย้อนกลับ 
	}else if(val==1){
		...
		alert("กรุณาระบุสาเหตุ");
			...
			callfrm("?option=la&task=main/report_6&index=4");   //page ประมวลผล
		}
	}
}
...
function goto_url2(val){
	if(val==0){
		...
		if(frm1.write_at.value == ""){
			...
			callfrm("?option=la&task=main/report_6&index=4.1");   //page ประมวลผล
		}
	}
}
...
function goto_url_update(val){
	if(val==0){
		...
		if(frm1.write_at.value == ""){
			...
			callfrm("?option=la&task=main/report_6&index=6");   //page ประมวลผล
		}
	}
}

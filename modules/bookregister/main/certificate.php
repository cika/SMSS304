...
<?php
if($_REQUEST['search_index']==1){
$sql="select *,bookregister_certificate.officer,bookregister_certificate.ms_id from bookregister_certificate left join person_main on bookregister_certificate.officer=person_main.person_id where  $_REQUEST[field] like '%$_REQUEST[search]%' order by year,register_number  limit $start,$pagelen "; 
}
else{
$sql="select *,bookregister_certificate.officer,bookregister_certificate.ms_id from bookregister_certificate left join person_main on bookregister_certificate.officer=person_main.person_id order by year,register_number limit $start,$pagelen "; 
}
$dbquery = mysqli_query($connect,$sql);
...
if(($result['officer'])==$user and ($delete==1) and ($result['quarantee']!=1)){
...
echo "<tr><td colspan='16'><b>การพิมพ์</b> 0=ไม่พิมพ์ 1=พิมพ์<td></tr>";	
echo "</table>";

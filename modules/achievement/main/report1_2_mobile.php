...
<CENTER>
<h3>คะแนนสอบ O-NET และผลการประเมิน</h3>
<?php
...
$sql = "select distinct ed_year from achievement_main where  test_type='1' and test_class='$_REQUEST[class_index]' order by ed_year desc ";
...
for($x=1;$x<4;$x++){
			if($ed_year==""){
			$strQuery = "select thai, math, science, social, english, health, art, vocation, score_avg from achievement_main where test_type='1' and test_class='$_REQUEST[class_index]' and ed_year='$year_ar[1]' and level='$x' ";
			}
			else{
			$strQuery = "select thai, math, science, social, english, health, art, vocation, score_avg from achievement_main where test_type='1' and test_class='$_REQUEST[class_index]' and ed_year='$ed_year' and level='$x' ";
			}
...
function goto_display(val){
	if(val==1){
		callfrm("?option=achievement&task=main/report1_2_mobile"); 
		}
}
</script>

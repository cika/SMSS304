...
<title>กำหนดผู้ใช้งาน</title>
  ...
  <?php
	...
	if(isset($_REQUEST['submitUP'])){	
				...
				$resultcpU=mysqli_query($connect,$sqlcpU); 
   			 	$rowscpU=mysqli_num_rows($resultcpU);
				if($rowscpU==0){
...
/*ADD Data*/
	if(isset($_REQUEST['submitAD'])){	
			...
			if($submitAD=="บันทึก")
			{
				...
				$resultcp=mysqli_query($connect,$sqlcp); 
   			 	$rowscp=mysqli_num_rows($resultcp);
				if($rowscp==0){
		...
		if($show==1)
		{
				...
				$result=mysqli_query($connect,$sqld);		
				echo "<script>window.location='?option=health&task=user'; </script>";
			}
		}
?>
...
<br>
<div align="center"> <h2>กำหนดผู้ใช้งาน</h2></div>
<?php
	if(isset($_REQUEST['show'])){	
	if($show==3){
?>
  ...
  <tr>
      ...
      <tr>
        ...
        <option value=""><-------------เลือก------------> </option>
          <?php
				...
				$resultmainP=mysqli_query($connect,$sqlmainP); 
   			    $n1=1;
				while($rowbsmainP=mysqli_fetch_array($resultmainP))
				{
      ...
      <tr>
        ...
        <option value=""><---------------เลือก---------------> </option>
          <?php
				...
				$resultroom=mysqli_query($connect,$sqlroom); 
   			    $Room=1;
				while($rowroom=mysqli_fetch_array($resultroom))
				{
					   $sqlCL="SELECT DISTINCT room FROM student_main  WHERE class_now='$rowroom[class_code]' order by room asc"; /*หาห้อง*/
						$resultCL=mysqli_query($connect,$sqlCL); 
						$ck_rows=mysqli_num_rows($resultCL);
						$CL=0;			
						while($recordCL=mysqli_fetch_array($resultCL))
					{	
									?>
	...
	?>
      ...
      <?php
						...
						$ck_STD=mysqli_num_rows($resultSTD);
						if($ck_STD==0){
						?>
      <tr>
     ...
     <table width="500" border="0" align="center">
      <tr>        
         ...
         <?php
				...
				$resultmainPup=mysqli_query($connect,$sqlmainPup); 
   			 	$rowbsmainPup=mysqli_fetch_array($resultmainPup);
				?>
      <tr>
        ...
        <?php
				...
				$resultmainPsw=mysqli_query($connect,$sqlmainPsw); 
				$rowbsmainPsw=mysqli_fetch_array($resultmainPsw);
				?>
      <tr>
	...
	?>
        ...
        <?php
		 $sqlroomA="SELECT  * FROM student_main_class WHERE class_code='$rowbsmainPup[per_position]'";
				$resultroomA=mysqli_query($connect,$sqlroomA); 
				$rowroomA=mysqli_fetch_array($resultroomA);
				?>
  ...
  <?php
		 ...
		 $bg ="";
		  while($recordS=mysqli_fetch_array($resultS)){
				...
				$rowL=mysqli_fetch_array($resultmainL);
				
				 $sqlroomSS="SELECT  * FROM student_main_class WHERE class_code='$recordS[per_position]'";
				$resultroomSS=mysqli_query($connect,$sqlroomSS); 
				$rowroomSS=mysqli_fetch_array($resultroomSS);
  ?>

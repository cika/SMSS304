...
<?php
  if(isset($_REQUEST['level']))  {
				...
				$recordB=mysqli_fetch_array($resultB);
		...
		if($submitC=="บันทึก"){
			...
			$sqlSV="INSERT INTO health_checking VALUES('','$student_id2','$student_number2','$year','$term','$level','$room','$checkN','$gum','$iodine','$tooth','$weight','$tall','$comment','$day','','$life','$push','$sit','$roll','$run','".$_SESSION['login_user_id']."')";
												$resultSV=mysqli_query($connect,$sqlSV);		
												$submitC="";										
														if($resultSV)
														{																			
...
<td height="30" bgcolor="#0066FF"><?php echo"$t2"; ?><font color="#FFFFFF">ตรววจสุขภาพนักเรียน</font></td>
  ...
  <tr>
    <td  valign="top"  height="260"><table width="1200" border="1" cellspacing="0" cellpadding="0">
	  ...
	  $num=1;
	  $bg="";
	while($recordCK=mysqli_fetch_array($resultCK)) /*student_main นักเรียน*/
	{
	  ...
	  ?>
		...
		$sqlSR="SELECT* FROM health_checking WHERE student_id='$student_id'&&room='$room'&&class_now=$level&&year_std=$year&&term_std=$term&&number_check=$checkN";
				...
				$checkSR=mysqli_num_rows($resultSR);
				$recordSR=mysqli_fetch_array($resultSR);
	         if($checkSR==0) {
				 /* ตรวจสุขภาพ*/
		?> 
      ...
      <tr valign="middle" bgcolor="<?php echo"$bg";?>">
        ...
        <td align="center"><input name="weight" id="weight" type="text" size="3" maxlength="6"  OnKeyPress="return chkNumber(this)" /></td>
        <td align="center"><input name="tall" id="tall" type="text" size="3" maxlength="6"  OnKeyPress="return chkNumber(this)" /></td>
        <td align="center"><select name="gum">
      ...
      <option value="รุนแรงต้องรีบรักษา">รุนแรงต้องรีบรักษา</option>
        ...
        <td align="center"><select name="tooth">
      <option value="ไม่ผุ">ไม่ผุ</option>
      <option value="ผุ" >ผุ</option>
    </select></td>
        <td align="center"><select name="iodine">
      <option value="ปกติ">ปกติ</option>
      <option value="เริ่มผิดปกติ">เริ่มผิดปกติ</option>
      <option value="ผิดปกติ">ผิดปกติ</option>
    </select> </td>
        <td align="center"><input name="life" type="text" size="3" maxlength="6" id="life" OnKeyPress="return chkNumber(this)"/></td>
        <td align="center"><input name="push" type="text" size="3" maxlength="6" id="push" OnKeyPress="return chkNumber(this)"/></td>
        <td align="center"><input name="sit" type="text" size="3" maxlength="6" id="sit" OnKeyPress="return chkNumber(this)" /></td>
        <td align="center"><input name="roll" type="text" size="3" maxlength="6" id="roll" OnKeyPress="return chkNumber(this)"/></td>
        <td align="center"><input name="run" type="text" size="3" maxlength="6" id="run" OnKeyPress="return chkNumber(this)"/></td>
        <td align="center"><textarea name="comment" rows="2" cols="15"></textarea></td>
          <td align="center">        

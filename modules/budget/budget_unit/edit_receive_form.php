...
function goto_url(val){
	if(val==0){
	callfrm("?option=budget&task=budget_unit/receive&page=<?php echo $_GET['page'];?>");   // page ย้อนกลับ 
	}else if(val==1){
	if(frm1.num.value == ""){
		alert("กรุณากรอกเลขที่ใบงวด");
			...
			callfrm("?option=budget&task=budget_unit/receive&index=3&page=<?php echo $_GET['page'];?>");   //page ประมวลผล
		}
	}
}
</script>

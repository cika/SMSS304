...
//ส่วนหัว
echo "<br />";
$check_date=date("Y-m-d");  //วันเช็ค

if(!(($index==1) or ($index==2) or ($index==5) or ($index==7))){
echo "<table width='100%' border='0' align='center'>";
echo "<tr align='center'>
	<td align=center><font color='#990000'><strong>บันทึกข้อมูลการมาเรียน ปีการศึกษา $txtyear_active</strong></font></td></tr>";
echo "<tr align='center'><td align=center><font color='#006666'>".thai_date(time())."</font></td></tr>";
echo "</table>";
}
//ส่วนฟอร์มรับข้อมูล
if($index==1){
...
//ลบข้อมูล
if($index==7){
			mysqli_query($connect,"delete from student_check_main where check_date='$_GET[datepicker]' and class_now='$_GET[class_now]' and room_now='$_GET[room_now]'");
$index=2;
}
...
echo "";
echo "<tr><td align='center'><font color='#990000'>บันทึกข้อมูลการมาเรียนของนักเรียน ชั้น$edu_level[$class_now]$room_now ปีการศึกษา $txtyear_active</font></td></tr>";
echo "<tr><td align='center'><font color='darkgreen'>".thai_date(time())."</font></td>
	</tr>";
echo "</table>";
	$room_now=$_GET['room_now'];
if(isset($_POST['bt_save']))
	{
		...
		$save_date=date("Y-m-d h:m:s");
			...
			{
					if(isset($_POST['chk_val'][$student_id[$l]])){
					$chk_val=$_POST['chk_val'][$student_id[$l]];
					}
					else{
					$chk_val="";
					}
				$dosqlchk=mysqli_num_rows(mysqli_query($connect,"select * from student_check_main where check_date='$datepicker' and student_id='$student_id[$l]' and student_check_year='$year_active'"));
				if($dosqlchk>0)
					{
...
if($pms==0 OR $_SESSION['login_status']>4){
	echo "<script>document.location='?option=student_check&task=main/check_mobile&index=1&class_now=$class_now&room_now=$room_now';</script>";
	}
$M=1;
$color="";
...
$color=(($M%2) == 0)?" class='even'":" class='odd'";
	...
	$return_vals=mysqli_fetch_array($query);
	$xz= $return_vals['check_val'];	
$comments=($xz)?"":"<img src=./modules/$_GET[option]/images/s_warn.png >";
?>
<TR <?php echo $color?>>
...
<?php }
?>
	<TD align="center"><?php echo $comments?></TD>
</TR>
<?php $M++;}
$BT_BACK=(isset($_POST['bt_save']))?"<INPUT TYPE=\"button\" onclick='location.href=\"?option=student_check&task=main/check_mobile\"' value=กลับไปหน้ารายการ>":"<INPUT TYPE=\"button\" onclick='location.href=\"$refer\"' value=กลับไปหน้าที่แล้ว>";
echo "<TR $color>
...
echo "<TR bgcolor='#FFCCCC' >
	<TD colspan=".(count($a_val_txt)+5)." align=right><INPUT TYPE=\"submit\" name=bt_save value=' [ บันทึกข้อมูล ] ' >&nbsp;&nbsp;
	<INPUT TYPE='button' name='smb' value='ลบข้อมูล' onclick='location.href=\"?option=student_check&task=main/check_mobile&index=7&class_now=$class_now&room_now=$room_now&datepicker=$check_date&t=$_GET[t]\"'>&nbsp;&nbsp;	<INPUT TYPE=\"reset\" value=' [ รีเซ็ต ] '>&nbsp;&nbsp;$BT_BACK
...
$url_link="option=student_check&task=main/check_mobile";  // 2_กำหนดลิงค์
...
While ($result = mysqli_fetch_array($dbquery))
	{
		...
		//$save_pic="<a href=?option=student_check&task=main/check_mobile&index=2&id=$id&page=$page&class_now=$class_now&room_now=$room_now&edu_type=".$result['edu_type']."&t=$edu_name><img src=images/save16x16.png border='0' alt='บันทึกข้อมูลการมาเรียนของห้องเรียนนี้' title='บันทึกข้อมูลการมาเรียนของห้องเรียนนี้' $Sat_Sun></a>";
		$save_pic="<a href=?option=student_check&task=main/check_mobile&index=2&page=$page&class_now=$class_now&room_now=$room_now&t=$edu_name><img src=images/save16x16.png border='0' alt='บันทึกข้อมูลการมาเรียนของห้องเรียนนี้' title='บันทึกข้อมูลการมาเรียนของห้องเรียนนี้' ></a>";

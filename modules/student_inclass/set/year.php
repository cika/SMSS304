...
<Table width='300' Border='0' Bgcolor='#Fcf9d8'>
	...
	$rec_date = date("Y-m-d");
if($_POST['updates']){
	$sqlCHK = mysqli_num_rows(mysqli_query($connect,"select student_check_year from student_check_year where student_check_year='$_POST[set_year]'"));
		//echo "ggg>".$sqlCHK;
		if($sqlCHK>0 && $_POST['set_year']==$_POST['set_year_old']){
			if($_POST['year_active']=='1'){
				$uquery = mysqli_query($connect,"update student_check_year set  year_active='0'");
			}
...
$edit_pic='<a href="?option=student_check&task=set/year&index=5&edit_year='.$student_check_year.'"><img src="images/edit.png" alt="แก้ไข" border="0">';
	...
	function goto_url(val){
		if(val==0){
			callfrm("?option=student_inclass&task=set/year");   // page ย้อนกลับ 
		}
	}
			function chkForm()
				{

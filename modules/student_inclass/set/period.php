...
if(isset($_POST['updates'])){
	$sqlCHK = mysqli_num_rows(mysqli_query($connect,"select student_check_year from student_inclass_year where student_check_year='$_POST[set_year]'"));
		//echo "ggg>".$sqlCHK;
		if($sqlCHK>0 && $_POST['set_year']==$_POST['set_year_old']){
			if($_POST['year_active']=='1'){
				$uquery = mysqli_query($connect,"update student_inclass_year set  year_active='0'");
			}
...
<Table width='400' Border='0' Bgcolor='#Fcf9d8'>
	...
	<Input Type='hidden' Name='set_year_old' value=$_GET[edit_year]>
		...
		&nbsp;&nbsp;&nbsp;</td>
	<td align='left'><INPUT TYPE='button' name='back' value='ย้อนกลับ' onclick='goto_url(0)' ></td></tr>
	</Table>
	</form>";
	}
...
$edit_pic='<a href="?option=student_inclass&task=set/period&index=5&edit_year='.$student_check_year.'&a1='.$num_period.'&a2='.$lunch_period.'&at='.$year_active.'"><img src="images/edit.png" alt="แก้ไข" border="0">';
	...
	function goto_url(val){
		if(val==0){
		callfrm("?option=student_inclass&task=set/period");   // page ย้อนกลับ 
		}
	}
			function chkForm()
				{

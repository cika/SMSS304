...
function getFullnameUser($x){
	...
	$FullnameUser=mysqli_fetch_assoc($query);
...
echo "<tr align='center'><td><font color='#006666' size='3'><strong>กำหนดผู้รับผิดชอบการบันทึกการเข้าเรียน <BR>ปีการศึกษา $txtyear_active</strong></font></td></tr>";
echo "</table>";
}
...
if($index==1){
echo "<table width='50%' border='0' align='center'>";
echo "<tr align='center'><td><font color='#006666' size='3'><strong>กำหนดผู้รับผิดชอบการบันทึกการเข้าเรียน</strong></font></td></tr>";
echo "</table>";
	...
	if($person_id==""){
		...
		if(mysqli_num_rows(mysqli_query($connect,"Select * from student_inclass_permission where class_now=$class_now and room_now=$room_now and person_id='$person_id' and student_check_year='$year_active'"))>0){
		echo"<br><center>
...
echo "<Center>";
echo "<Font color='#006666' Size=3><B>กำหนดผู้รับผิดชอบการบันทึกการเข้าเรียน  $edu_level[$class_now]$room_now";
...
echo "<Tr bgcolor='#FFCCCC'><Td  align='center' style='font-weight:bold'>ที่</Td><Td  align='center' style='font-weight:bold'>รหัสประจำตัว</Td><Td  align='center' style='font-weight:bold'>ผู้รับผิดชอบ</Td><Td  align='center' style='font-weight:bold'>การทำรายการ</Td></Tr>";
			...
			$pmsquery = mysqli_query($connect,$sql_pms); 
			if(mysqli_num_rows($pmsquery)>0){
				$p=1;
				while($pms=mysqli_fetch_assoc($pmsquery)){
					$x=$pms['person_id'];
...
<INPUT TYPE="hidden" name="edu_type" value="<?php echo $edu_type;?>">
	...
	<option value=''>เลือกผู้ใช้จากรายการ</option>
		<?php
				$sqlPersonMain="Select * from person_main where status = 0";
				$query_PM=mysqli_query($connect,$sqlPersonMain);
				if(mysqli_num_rows($query_PM)>0){
					while($r_PM=mysqli_fetch_assoc($query_PM))
					{
...
$sql="SELECT `student_main`.`id` ,`student_main`.`class_now` , `student_main`.`room`   FROM student_main GROUP BY `student_main`.`class_now` , `student_main`.`room`  LIMIT $start,$pagelen";
$dbquery = mysqli_query($connect,$sql);#echo $sql;echo $page;
...
While ($result = mysqli_fetch_array($dbquery))
	{
		...
		$edit_pic="<a href=?option=student_inclass&task=set/permission&index=5&id=$id&page=$page&class_now=$class_now&room_now=$room_now&edu_type=$edu_name><img src=images/edit.png border='0' title='แก้ไขผู้รับผิดชอบการบันทึกการเข้าเรียนของห้องเรียนนี้' alt='แก้ไขผู้รับผิดชอบการบันทึกการเข้าเรียนของห้องเรียนนี้'></a>";
		$save_pic="";
		$edit_pic=($year_active=="")?"<img src=images/edit.png border='0' title='แก้ไขผู้รับผิดชอบการบันทึกการเข้าเรียนของห้องเรียนนี้' alt='แก้ไขผู้รับผิดชอบการบันทึกการเข้าเรียนของห้องเรียนนี้'>":$edit_pic;

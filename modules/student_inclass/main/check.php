...
echo "<tr align='center'>
	<td align=center><font color='#990000' size='3'><strong>บันทึกข้อมูลการเข้าเรียนของนักเรียน ปีการศึกษา $txtyear_active</strong></font>
...
echo "";
echo "<tr>
	<td align='left'><font color='#990000' size='4'>บันทึกข้อมูลการเข้าเรียนของนักเรียน ชั้น$edu_level[$class_now]$room_now ปีการศึกษา $txtyear_active</font></td>
...
echo "<TR $color>
	...
	<TD>$result[prename] $result[name]  $result[surname] </TD>";
			...
			$sql_return_vals="Select * From student_inclass_main where student_id='$result[student_id]' and check_date='$check_date' and 	student_check_year='$year_active'";
...
for($t=0;$t<$num_period;$t++)	{
		...
		echo"<TD width=53>";
				if($t==$lunch_period){
				echo "พักกลางวัน <INPUT TYPE='hidden' name='Period[".($M-1)."][$t]' value='T'></TD>\n";
				}
				else{
		...
		<?php				
									...
									for($i=0;$i<count($a_val_txt);$i++)	{	
												if($yval[1]==$a_val[$i]){
												echo "<option value='".$a_val[$i]."' selected class='".$a_val[$i]."'>".$a_val_txt[$i]."</option>\n";
												}
												else{
												echo "<option value='".$a_val[$i]."'  class='".$a_val[$i]."'>".$a_val_txt[$i]."</option>\n";
												}
									}
...
While ($result = mysqli_fetch_array($dbquery))
	{
		...
		//$save_pic="<a href=?option=".$_GET['option']."&task=main/check&index=2&id=$id&page=$page&class_now=$class_now&room_now=$room_now&edu_type=".$result['edu_type']."&t=$edu_name><img src=images/save16x16.png border='0' alt='บันทึกข้อมูลการเข้าเรียนของห้องเรียนนี้' title='บันทึกข้อมูลการเข้าเรียนของห้องเรียนนี้' $Sat_Sun></a>";
		$save_pic="<a href=?option=".$_GET['option']."&task=main/check&index=2&id=$id&page=$page&class_now=$class_now&room_now=$room_now&t=$edu_name><img src=images/save16x16.png border='0' alt='บันทึกข้อมูลการเข้าเรียนของห้องเรียนนี้' title='บันทึกข้อมูลการเข้าเรียนของห้องเรียนนี้'></a>";
		$report_pic="<a href=?option=".$_GET['option']."&task=main/report_room&index=1&class_now=$class_now&room_now=$room_now><img src=images/browse.png border='0' title='ดูรายงานข้อมูลการเข้าเรียนของห้องเรียนนี้' alt='ดูรายงานข้อมูลการเข้าเรียนของห้องเรียนนี้'></a>";
		$color=(($M%2) == 0)?" class='even'":" class='odd'";
...
if($index==2){
echo "&nbsp;&nbsp;&nbsp;<FONT SIZE='2' COLOR='#333333'><b>คำอธิบาย</b></FONT>&nbsp;&nbsp;<FONT SIZE='2' COLOR='#FF0000'>ขาด</FONT> หมายถึง ผู้เรียนไม่มาโรงเรียน  <FONT SIZE='2' COLOR='#FF0000'>ไม่เข้าเรียน</FONT> หมายถึง ผู้เรียนมาโรงเรียน แต่ไม่เข้าเรียนในคาบเรียน ";
}
?>

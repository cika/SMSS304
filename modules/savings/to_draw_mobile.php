			...
			if(isset($_REQUEST['student'])){
				...
				if($student!="")
					...
					/*ชื่อ*/	
						...
						$resultCKN=mysqli_query($connect,$sqlCKN); 
						$recordCKN=mysqli_fetch_array($resultCKN);
						$level=$recordCKN['class_now'];
						$room=$recordCKN['room'];
						$student_number=$recordCKN['student_number'];
						/*ข้อมูลเงิน*/	
						$sqlCK="SELECT* FROM savings_money  WHERE std_id='$student'";
						$resultCK=mysqli_query($connect,$sqlCK); /* AND year_past='$Year_bs'*/
						...
						$resultSM=mysqli_query($connect,$sqlSM);  /*&&year_past='$Year_bs'*/
						...
						$resultSM2=mysqli_query($connect,$sqlSM2);  /*&&year_past='$Year_bs'*/
			...
			if(isset($_REQUEST['DrawCK']))	{
				...
				$rowmenu=mysqli_fetch_array($resultmenu);
				 if($rowmenu['per_draw']==1) {
											...
											{																			
											echo "<script>alert('จำนวนเงินไม่เพียงพอสำหรับการถอนเงิน กรุณาตรวจสอบใหม่ ');window.location='?option=savings&&task=to_draw_mobile&&student=$student&&year=$Year_bs&&level=$level';</script>";
											}else{
								...
								$resultSave=mysqli_query($connect,$sqlSave);	

									if($resultSave){
											echo"		<table align=\"center\" width='100%' border='1' cellspacing='0' cellpadding='0'  bordercolor='#99CCFF' bgcolor='#FFFFFF'>
												  <tr>
...
"; 						$sqlCKN2="SELECT* FROM student_main  WHERE student_id='$student'";
						$resultCKN2=mysqli_query($connect,$sqlCKN2); 
						$recordCKN2=mysqli_fetch_array($resultCKN2);
						
				$sqlmain1R="SELECT* FROM student_main_class WHERE class_code='$level'";
				$resultmain1R=mysqli_query($connect,$sqlmain1R); 
				$rowmain1R=mysqli_fetch_array($resultmain1R);
						echo"
												  <tr>
													<td><br>
						...
						<div align=\"center\"><a href=\"?option=savings&&task=to_draw_mobile\" title='ย้อนกลับ'><img src='modules/savings/iconS/bk.gif' border='0'></a></div>
												<br>
															<br>
																		</td>
												  </tr>
        ...
        <?php 
		 if(!isset($_REQUEST['DrawCK'])){
			 if(isset($level)){
				...
				$resultmain1=mysqli_query($connect,$sqlmain1); 
				$rowmain1=mysqli_fetch_array($resultmain1);
...
<td width="100%" height="30" bgcolor="#0066FF"><font color="#FFFFFF"><?php echo $t2;?>
  ...
  <tr>
    <td>
     ...
     <form name="form1" id="form1" action="?option=savings&&task=to_draw_mobile" method="POST" enctype="multipart/form-data"  onSubmit="return checkfor(this)">
	<tr>	   
			<td align="left">&nbsp;&nbsp;ปีการศึกษา :
			  <input name="year2" type="text"   value="<?php echo $Year_bs;?>" size="4" maxlength="4"  class="colorbk"/><font color="#FF0000"> *</font></td>
	</tr>
        ...
        <td>   
         <?php 
		  if(isset($level)){
                ...
                ?>
          &nbsp;&nbsp;ชั้น : <select name="level_TOP" id="level_TOP" onChange = "ajaxCall()" class="colortext">
           <?php  if($level=="") {?>
         ...
         <?php
			 ...
			 $resultmenuP=mysqli_query($connect,$sqlmenuP); 
				$wn=1;
				while($rowmenuP=mysqli_fetch_array($resultmenuP))
				{
...
<tr>
        <td align="left">
			 ...
			 if(isset($student)){
				...
				$resultmain12=mysqli_query($connect,$sqlmain12); 
				$rowmain12=mysqli_fetch_array($resultmain12);
				  }
                ?>
        &nbsp;&nbsp;นักเรียน : <Select  name='student' id='student' size='1'  class="colortext">
...
<tr>	   
        <td align="center"><input type="submit" name="submitH"  value="ตกลง"   /></td>
</tr>
  ...
  <tr>
	...
	<table width="100%" border="0">
      <tr>
        <td width="118" rowspan="3" align="center" valign="middle"></td>
  ...
  <tr>
	...
	?>
      <form name="formSave" action="?option=savings&&task=to_draw_mobile" method="post" enctype="multipart/form-data">
      <tr bgcolor="#FEE2FC">

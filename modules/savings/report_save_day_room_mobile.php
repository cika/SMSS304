  ...
  <tr>
	...
	<table width="100%" border="0">
      <tr>
        ...
        <td width="145" rowspan="3" align="center" valign="middle"><a href="?option=savings&task=report_all_mobile&dateInput=<?php echo $dateInput;?>" title="ย้อนกลับ"><img src='modules/savings/iconS/bk.gif'  border='0'></a></td>
        </tr>
        <?php
		if(isset($level)){
	...
	while($recordCK=mysqli_fetch_array($resultCK))
		{	
						...
						$resultSM=mysqli_query($connect,$sqlSM); 
						 ...
						 $moneySave=$recordSM['sumsave'];  /*รวมยอดฝาก ณ ปัจจุบัน ณ วันที่ ที่เลือก*/
...
$sqlSM2="SELECT SUM(amount_money) AS sumdraw FROM savings_money  WHERE std_id='$std_ckid'&&day_act='$dateInput'&&acc_type='2'";
						$resultSM2=mysqli_query($connect,$sqlSM2); 
						 ...
						 $moneysumdraw=$recordSM2['sumdraw']; /*รวมยอดถอน ณ ปัจจุบัน ณ วันที่ ที่เลือก*/
						 
			/********************คำนวณหายอดคงเหลือสุทธิ*************/

	$sqled="SELECT SUM(amount_money) AS saveed FROM savings_money  WHERE std_id='$std_ckid' AND  office <='$dateInputBT' AND acc_type='1'";
						$resulted=mysqli_query($connect,$sqled);  
						 $recorded=mysqli_fetch_array($resulted);
						 $saveed=$recorded['saveed'];  /*รวมยอดฝากทั้งหมด*/

$sqled2="SELECT SUM(amount_money) AS sumdrawed FROM savings_money  WHERE std_id='$std_ckid' AND office <='$dateInputBT' AND acc_type='2'";
						$resulted2=mysqli_query($connect,$sqled2); 
						 $recorded2=mysqli_fetch_array($resulted2);

        ...
        <td height="35" align="center">เลือก ปีการศึกษา : 
                  ...
                  <option value="<?php echo $pasted;?>"><?php echo $pasted;?></option>
					<?php   
						...
						$resultpast=mysqli_query($connect,$sqlpast); 
						$past=1;			
						while($rowspast=mysqli_fetch_array($resultpast))
						{
     ...
     <?php 
   			...
   			$sqlSM="SELECT DISTINCT room FROM student_main  WHERE class_now='$recordCK[class_code]' AND status!='0' order by room ASC"; /*หาจำนวนห้อง*/
					...
					$sqlsd="SELECT  * FROM student_main  WHERE class_now='$recordCK[class_code]' AND status!='0' AND room='$recordSM[room]'"; /*หาคนทั้งหมดในห้องนั้นๆ*/
						...
						$check_room_sd=mysqli_num_rows($resultsd);		/* จำนวนนักเรียนทั้งหมด status!=0*/			
						$person=1;			
					...
					if($recordsd['status']!=0){
						...
						$resultssave = mysqli_query($connect,$sqlsave); 
						$check_room_save = mysqli_num_rows($resultssave);	/* จำนวนนักเรียนที่ฝากเงิน*/			
						...
						$resultsdraw = mysqli_query($connect,$sqldraw); 
						$check_room_draw = mysqli_num_rows($resultsdraw);	/* จำนวนนักเรียนที่ถอนเงิน*/	
				...
				$recordbb=mysqli_fetch_array($resultbb);
					...
					$person++; /*หาคนทั้งหมดในห้องนั้นๆ*/
						...
						$total_sum_saveX = $total_sum_saveX + $total_sum_save; /* รวมยอดเงินสุทธิที่ฝากทั้งหมด*/
						$total_sum_drawX =  $total_sum_drawX + $total_sum_draw;  /* รวมยอดถอนสุทธิเงินทั้งหมด*/
						
						$total_sumX = $total_sumX + $total_sum;  /*ยอดเงินคงเหลือ*/
						$check_room_sdT = $check_room_sdT + $check_room_sd; /*จำนวนนักเรียนทั้งหมด*/
						
						$check_room_save_total= $check_room_save_total + $check_room_save; /*จำนวนครั้งที่ฝาก*/
						$check_room_draw_total= $check_room_draw_total + $check_room_draw; /*จำนวนครั้งที่ถอน*/
      ...
      <tr bgcolor="<?php echo $bg;?>"  onMouseOver="this.className='menu-over'" onMouseOut="this.className='menu'" class="menu">
        ...
        <td align="right" valign="middle"><?php echo number_format($total_sum_save,2,'.',',');?>&nbsp;</td>
        <td align="center" valign="middle"><?php echo $check_room_draw_total;?>&nbsp;</td>
        <td align="right" valign="middle"><?php echo number_format($total_sum_draw,2,'.',',');?>&nbsp;</td> 
